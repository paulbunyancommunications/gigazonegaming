...
class MakeIndividualSignUpPost extends Migration
{
    ...
    public function up()
    {
        $post = DB::table('wp_posts')
            ->where('post_name', 'individual-sign-up')
...
VALUES
	(1, '" . $now . "', '" . $now . "', '[individual-sign-up new_line=\",\" delimiter=\"|\" questions=\"Your Name,Your LOL Username,Your Email Address|email,Your Phone|tel\" inputs=\"your-name|name,your-email-address|email\"]Please fill out the form below to let us know that your are interested in participating in the event but don&#39;t have a team to compete with. We will try and find you a team[/individual-sign-up]', 'Individual Sign Up', '', 'publish', 'open', 'open', '', 'individual-sign-up', '', '', '" . $now . "', '" . $now . "', '', " . $parent->ID . ", 'http://gigazonegaming.local/auto-draft/', 0, 'post', '', 0);";
            DB::insert($query);
        }
    }
